...
public class MyDOT2FSM {
    ...
    public static void main( String[] args) throws Exception 
    {
    	...
    	MyDOT2FSM fsm = new MyDOT2FSM();
   		if (args[0].compareTo(helpFlag) == 0)
		{
			System.out.println("prog [-dot|-easyFSM] fsm-input-file  [thisrole] [package-name]");
			return;
		} else {
    		fsm.m_dotProcessing = (args[0].compareTo(dotProcessingFlag));
    		fsm.m_easyFSMProcessing = (args[0].compareTo(easyFSMProcessingFlag));
		}
    	if (args.length >= 1)
    	{
        	fsm.setInboundDoc(fsm.readFileIntoString(args[1]));
    	}
    	if (args.length >= 2)
